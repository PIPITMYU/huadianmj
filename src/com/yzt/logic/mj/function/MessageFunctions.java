...
public class MessageFunctions extends TCPGameFunctions {
	...
	public static Map<String, Object> getCurrentUserInfo(Player player, RoomResp room) {
		...
		//自己根据roominfo去判断
		if(room != null && room.getPlayType().equals(1) && (room.getState() == Cnst.ROOM_STATE_GAMIING || room.getState() == Cnst.ROOM_STATE_PIAO)){
			if(player.getPiao()!=null){
				currentUserInfo.put("piao", player.getPiao());
			}
		}
		if(room != null && player.getPlayStatus() == Cnst.PLAYER_STATE_GAME){
			...
			currentUserInfo.put("baoLouHu", player.isBaoLou()?2:1);
	 ...
	 * @param action 动作实体
	 */
	public static void interface_100104(RoomResp room, List<Player> players,Integer interfaceId,Action action,boolean huanBao) {
		...
		for (Player p : players) {
			...
			//听完牌后的提示
//			if(action.getActionId()>=174 && action.getActionId()<=230 && p.getUserId().equals(room.getNextActionUserId())){
//				info.put("tingList", p.getTingList());
//			}
			if(room.getState() != Cnst.ROOM_STATE_XJS){
				if(room.getNextAction().contains(501) || room.getNextAction().contains(-1)){
					...
					if(p.getUserId().equals(room.getNextActionUserId())){
						if(hasTingAndBaoLou(room)){
							//不给他们发听的动作先
							List<Integer> newAction = new ArrayList<Integer>();
							for(Integer a : room.getNextAction()){
								if(a<=230 && a>=174){
									continue;
								}
								newAction.add(a);
							}
							info.put("nextAction", newAction);
						}else{
							info.put("nextAction", room.getNextAction());						
						}
						info.put("nextActionUserId", room.getNextActionUserId());
	...
	/**
	 * 如果房间选项是必须暴露胡 这时只给报喽不给报听
	 * @param room
	 * @return
	 */
	public static  boolean hasTingAndBaoLou(RoomResp room){
		if(room.getBaoLouHu() == 2){
			return false;
		}
		boolean hasTing = false;
		boolean hasBaoLou = false;
		for(Integer action:room.getNextAction()){
			if(action == 231){
				hasBaoLou = true;
			}
			if(action>=174 && action <=230){
				hasTing = true;
			}
		}
		return hasTing && hasBaoLou;
	}
}
