...
//  		BackFileUtil.save(100103, room, null, redisRecord,null);
    		...
    		roomSave.put("baoLouHu", String.valueOf(room.getBaoLouHu()));
...
//		}
	 ...
	 * 大厅-战绩造数据
	 */
	public static void interface_100400(WSClient channel,
		...
		for (int i = 0; i < num; i++) {
			...
			// 小局结算信息 回放用
			RedisUtil.hmset(Cnst.REDIS_PLAY_RECORD_PREFIX.concat(key),
					roomSave, Cnst.TEST_PLAYOVER_LIFE_TIME);
		}
	 ...
	 * 俱乐部玩家战绩造数据
	 */
	public static void interface_100402(WSClient channel,
			Map<String, Object> readData) {
		Integer interfaceId = StringUtils.parseInt(readData.get("interfaceId"));
		Integer userId = StringUtils.parseInt(readData.get("userId"));
		...
		String userKey = Cnst.REDIS_CLUB_PLAY_RECORD_PREFIX_ROE_USER + clubId
				+ "_" + userId + "_" + redisDate;
		long currentTimeMillis = System.currentTimeMillis();
		for (int i = 0; i < num; i++) {
			...
			// 小局结算信息 回放用
			// 更新redis 缓存
			RedisUtil.hmset(Cnst.REDIS_CLUB_PLAY_RECORD_PREFIX.concat(key),
					roomSave, Cnst.PLAYOVER_LIFE_TIME);
		}
		Map<String, Object> info = new HashMap<String, Object>();
		JSONObject result = MessageFunctions.getJSONObj(interfaceId, 1, info);
		MessageUtils.sendMessage(channel, result.toJSONString());
	 ...
	 * 大厅代开历史造数据
	 */
	public static void interface_100401(WSClient channel,
			Map<String, Object> readData) {
		Integer interfaceId = StringUtils.parseInt(readData.get("interfaceId"));
		Integer userId = StringUtils.parseInt(readData.get("userId"));
		// 要造的数量
		...
		for (int i = 0; i < num; i++) {
			...
			// String key = room.getRoomId()+"-"+room.getCreateTime();
			String key = roomId + "-" + (currentTimeMillis + i);
			String key1 = Cnst.REDIS_PLAY_RECORD_PREFIX_ROE_DAIKAI
